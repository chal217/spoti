...
echo ''.$colors->getColoredString("
        _____ ____  ____  __________________  __
       / ___// __ \/ __ \/_  __/  _/ ____/\ \/ /
       \__ \/ /_/ / / / / / /  / // /_     \  /
      ___/ / ____/ /_/ / / / _/ // __/     / / 
     /____/_/    \____/ /_/ /___/_/       /_/ ", "green").''.PHP_EOL;
echo PHP_EOL.'     '.$colors->getColoredString("Tools Account Creator Spotify and Get Cookie | RCC STORE | ST4RZ", "white").''.PHP_EOL;                                                                      
echo PHP_EOL.'----------------------- '.$colors->getColoredString("MENU", "green").' -----------------------'.PHP_EOL.PHP_EOL;
echo '-> '.$colors->getColoredString("Berapa akun : ", "blue");
$u = trim(fgets(STDIN));
for ($i = 0; $i < $u;$i++){
$run = generateAccount($colors);
}
...
function randDomain(){
	$arraymail = array('osyduck.com','gmail.com');
...
function randomUser($ua){
	...
	$param = " number_generate=1&submit=Generate";
	$headers = array();
...
function randomUseragent(){
	$browser = array('Mozilla','Firefox','Edge');
	...
	$os = array('Windows','Linux','Macintosh');
    ...
    $edgeVersion = rand(11,99). ".". rand(1,9). ".". rand(1111,9999). ".". rand(10,99);
	$safariVersion = rand(111,999). ".". rand(11,99);
	$browserVersion = rand(1,9). ".". rand(0,9);
	return $ua = "$browser/$browserVersion ($os; Android $androidVersion; $mobile) AppleWebKit/$safariVersion (KHTML, like Gecko) Chrome/$chromeVersion Safari/$safariVersion Edge/$edgeVersion";
}
...
function generateAccount($colors){	
	requirement:
	$load = getConfig();
	//var_dump($load);
	$apikey = $load['Spotify']['apikey'];
	$pass = $load['Spotify']['password'];
	ceckpoint:
    $ua = randomUseragent();
	$username = randomUser($ua);
	$domain = $load['Spotify']['domain'];
	$uname = str_replace(' ','',$username);
	$randStr = rand(10,999);
	$names = "".substr(str_shuffle('abceefgihjklmnopqrstuvwxyz'), 0, 5);
	...
	if(preg_match('/login_token/i',$register)){
		...
		//echo "\nϟ ($email) Process Claim ";
		} else if(preg_match('/already_exists/i',$register)){
        echo "-> Sudah ada akun untuk email ini.\n";
        echo "[✓] Mencoba daftar ulang\n";
		goto ceckpoint;
	}
	...
	if($x['isAnonymous'] == false){
	    ...
	    echo PHP_EOL.'>> '.$colors->getColoredString("$email", "green").' | Berhasil';
	    file_put_contents("akun.txt", "{$email}|{$pass}|$token\n", FILE_APPEND);
	}
        echo PHP_EOL.PHP_EOL.'>> '.$colors->getColoredString("Done Create Account Spotify", "green").' <<'.PHP_EOL.PHP_EOL;
class Colors {
